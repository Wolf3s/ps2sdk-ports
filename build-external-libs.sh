...
$FETCH 0.5.2 https://github.com/chexum/genromfs.git
...
pushd build/isjpcm
sed -i -e 's|$(MAKE) -C ee install||' Makefile
sed -i -e 's|$(MAKE) -C iop install||' Makefile
sed -i -e 's|rm -f -r $(IOP_OBJS_DIR) $(IOP_BIN_DIR)||' iop/Makefile
popd
...
build_ee theora
make_ps2 isjpcm
install -m755 isjpcm/ee/lib/libsjpcm.a $PS2SDK/bin
install -m755 isjpcm/ee/src/*.h $PS2SDK/bin
install -m755 isjpcm/iop/bin/isjpcm.irx $PS2SDK/bin
